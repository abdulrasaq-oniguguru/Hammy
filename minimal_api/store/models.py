...
# Simplified ProductChoices replacement for minimal API
class ProductChoices:
    """Minimal ProductChoices for API - only essential constants"""
    SHOP_TYPE = SHOP_TYPE
    MARKUP_TYPE_CHOICES = MARKUP_TYPE_CHOICES
    COLOR_CHOICES = []
    DESIGN_CHOICES = []
    CATEGORY_CHOICES = []

    @staticmethod
    def get_display_value(choices, value):
        """Simple display value getter"""
    ...
    @staticmethod
    def get_all_colors_with_custom(model_class):
        """Stub method - returns empty list for minimal API"""
        return []

    @staticmethod
    def get_all_designs_with_custom(model_class):
        """Stub method - returns empty list for minimal API"""
        return []

    @staticmethod
    def get_all_categories_with_custom(model_class):
        """Stub method - returns empty list for minimal API"""
        return []
...
class TransferItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.transfer.transfer_type == 'internal':
            # For internal transfers, validate location and shop
            ...
            if self.product.shop != self.transfer.from_shop:
...
class PreOrder(models.Model):
    ...
    brand = models.CharField(max_length=100)  # Product brand - main identifier
...
class Delivery(models.Model):
    ...
    DELIVERY_COST_CHOICES = [
        (0, 'No Delivery (Pickup)'),
        (1000, 'Standard Delivery - ₦1,000'),
        (1500, 'Standard Delivery - ₦1,500'),
        (2000, 'Standard Delivery - ₦2,000'),
        (2500, 'Express Delivery - ₦2,500'),
        (3000, 'Standard Delivery - ₦3,000'),
        (3500, 'Priority Delivery - ₦3,500'),
        (4000, 'Priority Delivery - ₦4,000'),
        (4500, 'Priority Delivery - ₦4,500'),
        (5000, 'Same Day Delivery - ₦5,000'),
        (5500, 'Same Day Delivery - ₦5,500'),
        (6000, 'Same Day Delivery - ₦6,000'),
        (6500, 'Same Day Delivery - ₦6,500'),
        (7000, 'Same Day Delivery - ₦7,000'),
        (7500, 'Same Day Delivery - ₦7,500'),
        (8000, 'Same Day Delivery - ₦8,000'),
        (8500, 'Same Day Delivery - ₦8,500'),
        (9000, 'Same Day Delivery - ₦9,000'),
    ]
...
class Receipt(models.Model):
    ...
    # CRITICAL: Removed auto_now_add=True to allow manual date setting from sync
    date = models.DateTimeField(null=True, blank=True)

    customer = models.ForeignKey('Customer', on_delete=models.SET_NULL, null=True, blank=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)

    # Track local database ID to prevent duplicates during sync
    local_receipt_id = models.IntegerField(null=True, blank=True, db_index=True, unique=True)
    ...
    def calculate_total(self):
        ...
        subtotal = sum(sale.total_price for sale in self.sales.all())
        self.subtotal = subtotal

        # Get discount from payment (once per receipt)
        discount = Decimal('0')
        ...
        if sales.exists():
            ...
            if hasattr(first_sale, 'payment') and first_sale.payment:
                ...
                if payment.discount_percentage:
                    discount = subtotal * (payment.discount_percentage / 100)
                else:
                    discount = payment.discount_amount or Decimal('0')

        # Formula: Subtotal - Discount + Delivery = Total
        self.total_with_delivery = subtotal - discount + self.delivery_cost
        return self.total_with_delivery
...
class Payment(models.Model):
    ...
    # Track local database ID to prevent duplicates during sync (ADDITIVE only)
    local_payment_id = models.IntegerField(null=True, blank=True, db_index=True, unique=True)
    ...
    def calculate_total(self):
        ...
        if self.pk:
            ...
            total = sum(sale.total_price for sale in self.sale_set.all())
            ...
            if hasattr(self, 'sale_set') and self.sale_set.exists():
                ...
                if sale.delivery:
                    final_amount += sale.delivery.delivery_cost
    ...
    def update_payment_status(self):
        ...
        self.total_paid = sum(method.amount for method in self.payment_methods.filter(status='completed'))
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(*args,
                     update_fields=['total_amount', 'discount_amount', 'total_paid', 'balance_due', 'payment_status',
                                    'completed_date'])
...
class Sale(models.Model):
    ...
    # CRITICAL: Removed auto_now_add=True to allow manual date setting from sync
    # This preserves the original sale date from local database
    sale_date = models.DateTimeField(null=True, blank=True)

    # Track local database ID to prevent duplicates during sync (ADDITIVE only)
    local_sale_id = models.IntegerField(null=True, blank=True, db_index=True, unique=True)
    ...
    def calculate_tax_amount(self, subtotal):
        ...
        if self.tax_type == 'percentage':
            if self.calculation_method == 'inclusive':
                ...
                tax_amount = subtotal - (subtotal / (1 + (self.rate / 100)))
            else:
                # Tax is added to the price
                # Tax = Subtotal * (rate/100)
                tax_amount = subtotal * (self.rate / 100)
        else:
            # Fixed amount tax
